...
지금 시점(Phase 6까지 완성)에 메이저 버전을 다운그레이드하면 새로운 호환성 문제가 생길 위험이 더 크다고 판단. **다운그레이드하지 않고 기술 부채로 기록만 남기고 진행.** 시간이 남으면 추후 별도 처리.
...
## 2026-06-23 | 홈→상세→예약 화면 이동 시 "Maximum update depth exceeded"

### 증상

홈 화면에서 숙소 카드를 눌러 상세로 이동한 다음, 거기서 "예약하기"를 누르면 예약 화면으로 넘어가지 못하고 크래시. 상세 화면 URL을 새로고침으로 바로 띄운 뒤 "예약하기"를 누르면 재현되지 않음 — 즉 홈 화면이 배경에 계속 떠 있는 상태에서만 발생.

### 원인

`booking/[roomId].tsx`에만 `useRootNavigationState()`(루트 네비게이터가 준비됐는지 확인하는 훅)와, 그 화면 안에서 직접 렌더링하는 `<Stack.Screen options={{...}}>`가 같이 있었음. `options`로 넘기는 객체는 매 렌더마다 새로 만들어지는 객체라서, 화면이 다시 렌더링될 때마다 내비게이션에 `setOptions`를 호출한다. 그런데 `useRootNavigationState()`는 앱 전체 내비게이션 상태가 바뀔 때마다 이 화면을 다시 렌더링시킨다 — 홈(탭 네비게이터)이 배경에 같이 떠 있으면 내비게이션 상태가 더 자주 바뀌고, 그 변화가 다시 이 화면을 리렌더링시켜서 `setOptions`를 또 호출하는 식으로 핑퐁이 반복되다가 React의 렌더링 횟수 제한에 걸려 크래시.

게다가 이 `useRootNavigationState()` 가드는 애초에 불필요했다 — 그 훅이 막으려던 "앱이 막 시작될 때 루트 레이아웃이 마운트되기 전에 네비게이션을 시도하는" 상황은, 항상 다른 화면에서 push로만 들어오는 `booking/[roomId].tsx`에서는 일어날 수 없는 상황이었다.

### 해결

`useRootNavigationState()`를 제거하고 `isRestoring` 가드만 남김. 헤더를 공통 컴포넌트(`AppHeader`)로 옮기면서 화면 안에서 `<Stack.Screen>`을 직접 렌더링하는 코드 자체도 없어짐.

### 교훈

- `useRootNavigationState()`(또는 비슷하게 "전역 내비게이션 상태"를 구독하는 훅)는 정말 필요한 화면(앱 부팅 시 첫 화면이 될 수 있는 화면)에만 써야 한다 — 모든 인증 가드 화면에 기계적으로 복사해 붙이면 안 된다
- "화면 안에서 직접 렌더링하는 `<Stack.Screen options={{...}}>`"와 "전역 내비게이션 상태를 구독하는 훅"을 같은 화면에 같이 두면, 한쪽의 변화가 다른 쪽을 다시 트리거하는 핑퐁 루프가 생길 수 있다
- 이런 버그는 "화면을 새로고침으로 바로 띄워서" 테스트하면 절대 안 드러난다 — 실제 사용자처럼 앱 안에서 여러 화면을 거쳐 들어가는 경로로 테스트해야 발견된다

---

## 2026-06-23 | 헤더 공통화 이후 일부 화면에 헤더 2줄(라우트 파일명 + AppHeader) 노출

### 증상

상세·예약·검색결과 화면에서 "accommodation/[id]"처럼 라우트 파일명이 그대로 제목으로 박힌 헤더 바가, 직접 만든 `AppHeader` 위에 하나 더 떠서 2줄로 보임.

### 원인

화면 안에서 직접 렌더링하던 `<Stack.Screen options={{...}}>`을 `AppHeader` 공통화 작업 때 지웠는데, 그 라우트들을 루트 `<Stack>`에 명시적으로 등록하면서 `headerShown: false`를 같이 주는 걸 빠뜨림. Expo Router는 `<Stack>` 안에 명시적으로 등록되지 않은 라우트도 자동으로 화면을 만들어주는데, 이때 기본값은 "헤더 보임 + 제목은 라우트 파일명"이라서 의도치 않은 헤더가 추가로 뜸.

### 해결

루트 `<Stack>`에 `screenOptions={{ headerShown: false }}`를 줘서 모든 라우트의 기본값 자체를 "헤더 없음"으로 바꾸고, 정말 네이티브 헤더가 필요한 화면(`modal`)만 개별적으로 `headerShown: true`를 다시 켬.

### 교훈

화면 단위로 `<Stack.Screen options>`를 지우는 리팩토링을 할 때는, 그 화면이 루트 네비게이터에 "명시적으로 등록돼 있는지"와 "기본값이 무엇인지"를 같이 확인해야 한다. 일부 라우트만 명시적으로 등록하고 나머지는 암묵적으로 맡기면, 등록 안 된 라우트들은 라이브러리의 기본값(이 경우 헤더 보임)을 그대로 따라간다.

---

## 2026-06-23 | 실기기에서 헤더·탭바가 상태바/홈 인디케이터에 가려짐(safe area 미처리)

### 증상

Expo Go로 실제 폰에서 확인하니 화면 상단·하단이 잘려 보임. 웹 미리보기에서는 전혀 안 보이던 문제.

### 원인

`AppHeader`/`BottomTabBar`/로그인 화면의 뒤로가기 버튼을 직접 만들면서, 기기마다 다른 상태바·노치·홈 인디케이터 영역(safe area)을 고려하지 않음. 네이티브 기본 헤더/탭바를 쓸 때는 이걸 자동으로 처리해줘서 그동안 신경 쓸 필요가 없었는데, 직접 만든 컴포넌트로 교체하면서 그 처리가 빠짐. 웹은 이런 영역 자체가 없어서(`insets`가 전부 0) 헤드리스 브라우저 검증으로는 절대 못 잡는 종류의 버그.

### 해결

`react-native-safe-area-context`의 `useSafeAreaInsets()`를 가져와서 헤더 상단/탭바 하단에 그 값만큼 패딩을 추가.

### 교훈

네이티브 헤더·탭바·네비게이션 같은 "OS가 알아서 처리해주던 것"을 커스텀 컴포넌트로 직접 만들 때는, 안전영역 처리도 같이 가져와야 한다는 걸 빠뜨리기 쉽다. 이런 버그는 웹 미리보기로는 원천적으로 검증이 안 되니, 화면 구조를 바꾸는 작업 후에는 실기기 확인이 필수라는 걸 보여주는 사례.

---

## 2026-06-23 | 실기기에서 상태바(시간·배터리) 아이콘이 안 보임

### 증상

폰에서 앱을 켜면 화면 맨 위 상태바에 시간·배터리 표시가 안 보임(완전히 없는 게 아니라, 흰 배경에 흰 아이콘이라 안 보이는 상태였음).

### 원인

`app/_layout.tsx`의 `<StatusBar style="auto" />`는 폰의 시스템 다크모드 설정에 따라 상태바 아이콘 색을 자동으로 light(흰색)/dark(검은색)로 바꾼다. 이 앱은 다크모드 자체를 지원하지 않고 항상 흰 배경이라, 폰이 다크모드일 때 `auto`가 흰색 아이콘을 선택하면서 흰 배경 위에 흰 아이콘이 겹쳐 안 보이게 됨.

### 해결

`style="dark"`로 고정 — 앱 배경이 항상 밝은색이므로 상태바 아이콘도 항상 어두운 색으로 고정.

### 교훈

`style="auto"`는 "지금 보고 있는 화면 배경"이 아니라 "시스템 다크모드 설정"을 기준으로 동작한다. 앱이 다크모드를 지원하지 않고 배경이 고정돼 있다면, 상태바 스타일도 그 배경에 맞춰 고정하는 게 맞다 — `auto`를 쓰면 시스템 설정과 앱 배경이 어긋나는 조합에서 이런 "보이긴 하는데 안 보이는" 버그가 생긴다.

---

## 2026-06-23 | 실기기(Expo Go)에서 Google 로그인 400: invalid_request

### 증상

폰의 Expo Go 앱으로 Google 로그인 버튼을 누르면, Google이 "액세스 차단됨: 승인 오류 / 400: invalid_request"를 띄움. 오류 세부정보 확인 결과 `redirect_uri=exp://192.168.0.175:8081`로 요청됨.

### 원인 조사 과정

1. 처음엔 "이 주소가 Google Cloud Console에 미등록"이라고 판단 → 등록 시도
2. Console의 "승인된 리디렉션 URI" 입력란이 자체적으로 거부: "공개 최상위 도메인(.com, .org 등)으로 끝나야 합니다" / "유효한 최상위 비공개 도메인을 사용해야 합니다"
3. 즉, 단순히 안 등록된 게 아니라 **Google이 "웹 애플리케이션" 타입 OAuth 클라이언트에는 `exp://IP:포트` 같은 비도메인 주소 자체를 등록받지 않음**(정책 차원의 차단)
4. `expo-auth-session@6.0.3`(현재 설치 버전)의 타입 정의를 확인한 결과, 예전에 이런 상황에서 쓰던 "Expo 인증 프록시"(`useProxy` 옵션, `auth.expo.io`라는 정식 도메인을 거쳐 가는 방식)가 이 버전에서 제거되어 있음을 확인
5. 결론: 일반 Expo Go 앱(스토어에서 받은, 커스텀 스킴 없는 범용 플레이어) 안에서는 Google OAuth를 끝까지 통과시킬 방법이 현재 패키지 버전 기준으로 없음 — 코드/설정 문제가 아니라 Expo Go라는 실행 방식 자체의 구조적 한계

### 해결 방향

웹/시뮬레이터 환경(`http://localhost`)에서는 이미 정상 동작 검증됨(Phase 12) — 이 경로는 영향 없음. 실기기에서 끝까지 동작시키려면 일반 Expo Go가 아니라, 앱 전용 스킴(`roomiapp://`)을 가진 커스텀 빌드(EAS Build, development client)가 필요 — 그 경우 Google OAuth도 "웹 애플리케이션" 클라이언트가 아니라 "Android" 클라이언트(패키지명+서명 인증서로 검증, 도메인 검증 없음)로 등록 가능해짐. `app.json`에 `android.package`(`com.roomi.app`) 추가, `expo-dev-client` 설치, `eas.json` 작성까지 완료, 실제 빌드는 사용자 Expo 계정으로 진행 예정.

### 교훈

- 에러 메시지(`invalid_request`)만 보고 "주소를 안 등록해서 그렇다"고 바로 단정하면 안 된다 — 등록을 시도해보니 그 자체가 거부되는 걸 보고서야 "이건 등록의 문제가 아니라 정책의 문제"라는 걸 알게 됨. 1차 진단이 틀렸을 가능성을 항상 열어두고, 다음 단계(등록 시도)에서 나온 새 정보로 진단을 갱신해야 한다
- "어디서는 된다더라"는 말은 정확히 같은 조건(같은 실행 방식·같은 플랫폼)에서 테스트한 게 맞는지 확인 전에는 반증으로 못 쓴다 — 실제로 확인해보니 그 사례는 웹 환경 테스트였을 가능성이 높았음
...
## 2026-06-25 | 배포된 웹사이트 동적 라우트 직접 접근 시 404

### 증상

`https://roomi-app-six.vercel.app/booking/:roomId`, `/accommodation/:id`처럼 동적 라우트 주소를 직접 입력하거나 외부 링크로 들어가면 404. 앱 안에서 카드를 눌러 들어가는 경우엔 문제없음.

### 원인

Expo 웹 정적 내보내기(`output: "static"`)는 동적 라우트를 실제 파라미터 값이 아니라 `[roomId].html`, `[id].html`처럼 꺾쇠괄호가 그대로 들어간 파일명으로 생성한다. 앱 내부 이동은 클라이언트 사이드 라우팅이라 서버에 새로 요청하지 않아 문제가 안 드러나지만, 직접 접근(새로고침·외부 링크)은 실제 서버 요청이 발생해서 주소와 파일명이 일치하지 않아 404가 난다.

### 해결

`vercel.json`에 `rewrites` 규칙을 추가해 패턴 매칭된 요청을 실제 파일로 연결:

```json
{
  "rewrites": [
    { "source": "/accommodation/:id", "destination": "/accommodation/%5Bid%5D.html" },
    { "source": "/booking/:roomId", "destination": "/booking/%5BroomId%5D.html" }
  ]
}
```

파일명의 꺾쇠괄호(`[`, `]`)는 URL에서 예약된 특수문자라 `destination`에 그대로 쓰면 매칭이 안 되고, URL 인코딩(`%5B`, `%5D`)으로 써야 정상 동작한다.

### 교훈

정적 호스팅에서 동적 라우트를 다룰 때는 "파일이 실제로 어떤 이름으로 만들어지는지"와 "그 이름을 라우팅 규칙에서 어떻게 참조해야 하는지"가 다를 수 있다 — 특수문자가 포함된 파일명은 규칙 작성 시에도 인코딩을 고려해야 한다.

---

## 2026-06-25 | Vercel 배포 시 사이트 전체가 404 (`.gitignore`의 `dist/` 제외)

### 증상

`vercel.json` 수정 후 재배포했는데도 홈 화면을 포함한 전체 사이트(`/`, 정적 자산 포함)가 404.

### 원인

프로젝트 루트(`roomi-app/`)에서 `vercel --prod`를 실행하면, 그 디렉터리의 `.gitignore`에 등록된 `dist/`가 배포 대상에서 통째로 제외된다. Vercel CLI는 기본적으로 배포 디렉터리의 `.gitignore` 항목을 업로드에서 자동으로 뺀다.

### 해결

배포 명령을 `dist/` 폴더 안에서 실행(`cd dist && vercel --prod`). 그 폴더는 자기 자신을 제외시킬 `.gitignore`가 없어 정상 업로드됨.

### 교훈

`.gitignore`는 "git에 안 올릴 파일"을 정하는 규칙이지만, 빌드 산물처럼 git에는 안 올리되 배포에는 포함해야 하는 폴더가 있다면 배포 도구가 그 규칙을 그대로 물려받는지 확인해야 한다.

---

## 2026-06-25 | 웹 배포 시 아이콘 이미지 깨짐 재발 (`assets/node_modules` 경로)

### 증상

`dist/`에서 정상 배포된 뒤에도 아이콘 이미지가 전부 깨짐.

### 원인

2026-06-24에 이미 발생했던 문제의 재발(해당 항목 참고) — 아이콘 폰트 에셋이 `assets/node_modules/...` 경로로 생성되고 Vercel이 경로에 `node_modules`가 포함되면 항상 제외시킴. `dist`는 `expo export`마다 새로 생성되어 그때의 수동 수정이 다음 배포에는 남아있지 않음.

### 해결

매번 손으로 고치는 대신 `roomi-app/scripts/fix-web-deploy.js`를 작성해 `assets/node_modules` → `assets/vendor` 이름 변경과 JS 번들 내 경로 문자열 치환을 자동화. `package.json`에 `"export:web": "expo export -p web && node ./scripts/fix-web-deploy.js"` 스크립트 추가.

### 교훈

빌드 산물에 매번 똑같이 적용해야 하는 수동 수정이 있다면, 그 수정 자체를 빌드 파이프라인의 일부(스크립트)로 만들어야 한다 — 기억에 의존한 반복 작업은 언젠가 빠진다.
