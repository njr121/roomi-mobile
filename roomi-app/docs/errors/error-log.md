...
## 2026-06-23 | `assets/images/pension-1.jpg` 파일 누락으로 앱 전체 빌드 실패

### 증상

`npx expo start`는 정상인데 브라우저에서 빈 화면(500), 서버 로그에 "Unable to resolve module @/assets/images/pension-1.jpg" 에러.

### 원인

`lib/typeImages.ts`와 `components/SplashScreen.tsx`가 `require("@/assets/images/pension-1.jpg")`로 그 파일을 정적으로 참조하는데, 실제 폴더에 파일이 없었음(나머지 15장은 정상 존재). `require()`는 빌드 시점에 파일이 실제로 있어야 하므로, 하나만 없어도 전체 번들링이 실패한다.

### 해결

임시로 두 파일에서 `pension-1.jpg` 참조를 제거(펜션 타입은 3장으로 운영). 실제 파일이 준비되면 제자리에 복구.

### 교훈

`require()`로 정적 임포트하는 에셋은 "이미지 하나 빠졌다"가 아니라 "앱 전체가 안 뜬다"로 나타난다. 이미지 같은 에셋 폴더를 외부에서 통째로 교체/추가할 때는 코드가 참조하는 파일명 목록과 실제 폴더 내용이 정확히 일치하는지 먼저 대조하는 게 안전하다.
