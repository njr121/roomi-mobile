...
더 정교한 패턴(Account 재사용, RefreshToken 회전)을 검토했으나, 구현 복잡도 대비 안정성을 우선해 단순 버전(JWT 1개, 회전 없음)으로 최종 확정.
...
## 260625 — 브랜치 동기화, 로그인 성능 진단

### 브랜치 동기화
- `feat/api`/`feat/app`에 `main`까지 합쳐지지 않은 커밋 정리: `feat/api`→`develop` → `feat/app`→`develop` → `develop`→`main` 순서로 PR 3건 생성·머지, 충돌 없음

### 로그인 후 메인 화면 이동 지연 — 원인 진단
- 모바일 Google 로그인 라우트(`roomi-api/app/api/auth/mobile/google/route.ts`)가 요청마다 구글 `tokeninfo` API 호출, Neon DB upsert, JWT 서명을 순차 처리
- 로그아웃·로그인 연속 2회 테스트 결과: 첫 호출은 서버리스 함수·DB 콜드 스타트로 추가 지연, 두 번째 호출도 구글 외부 API 호출 + DB 쿼리 왕복으로 기본 지연이 남음
- 결론: 코드 결함이 아닌 서버리스 아키텍처(Vercel+Neon) 구조에서 오는 지연 — 현재는 알려진 성능 이슈로 기록만 하고 별도 수정은 보류

## 260625 (이어서) — 예약 캘린더 기간 표시 버그 수정

### 버그 수정
- 증상: 예약 화면 캘린더에서 체크인~체크아웃 사이 날짜가 색칠 안 되고 시작·끝 날짜만 표시됨
- 원인: `react-native-calendars`의 `markingType="period"`는 범위 안의 모든 날짜를 `markedDates`에 개별 등록해야 이어진 띠로 표시되는데, 기존 코드는 체크인·체크아웃 두 날짜만 등록하고 있었음
- 해결: `app/booking/[roomId].tsx`에 `getDateBetween` 함수 신규 작성(두 날짜 사이 모든 날짜를 배열로 반환), `markedDates`를 이 배열 기반으로 `reduce`로 생성하도록 변경 — 배열의 첫/마지막 인덱스만 `startingDay`/`endingDay`를 `true`로 설정
- `npx tsc --noEmit` 오류 0개, 로컬에서 캘린더 띠 표시 정상 확인

## 260625 (이어서) — 웹 배포 동적 라우트 404 + 배포 자동화 정리

### 동적 라우트 404 수정
- 증상: 배포된 웹사이트에서 `/booking/:roomId`, `/accommodation/:id` 같은 동적 라우트를 직접 주소로 들어가면 404(앱 안에서 클릭 이동은 정상 — 클라이언트 사이드 라우팅이라 서버 요청이 안 생겨서 문제가 안 드러남)
- 원인: Expo 웹 정적 내보내기가 동적 라우트를 `[roomId].html`, `[id].html`처럼 꺾쇠괄호가 그대로 들어간 파일명으로 생성하는데, 실제 요청 주소(`/booking/실제ID값`)와 파일명이 일치하지 않아 매칭 실패
- 해결: `roomi-app/public/vercel.json` 신규 작성 — `rewrites`로 `/booking/:roomId` 등 패턴 요청을 해당 파일로 연결. 파일명의 꺾쇠괄호는 URL 인코딩(`[` → `%5B`, `]` → `%5D`)해야 정상 매칭됨을 확인. `public/` 폴더에 둬서 `expo export`할 때마다 자동으로 `dist/`에 복사되게 함(매번 손으로 만들 필요 없게)

### 배포 자체가 막혔던 문제 발견·수정
- 증상: 위 수정 후 재배포했는데도 홈 화면을 포함한 전체 사이트가 404
- 원인: `roomi-app/`(프로젝트 루트)에서 `vercel --prod`를 실행하면, 그 폴더의 `.gitignore`에 있는 `dist/`가 배포 대상에서 통째로 제외됨(Vercel CLI가 `.gitignore` 항목을 자동으로 업로드에서 뺌)
- 해결: `dist/` 폴더 안으로 이동해서 배포(`cd dist && vercel --prod`) — 그 폴더엔 자신을 제외시킬 `.gitignore`가 없어서 정상 업로드됨

### 아이콘 이미지 깨짐 재발 + 자동화
- 증상: `dist/`에서 재배포 후 사이트는 정상 로드됐지만 아이콘 이미지가 전부 깨짐
- 원인: 0624에 이미 겪었던 문제 재발 — 아이콘 폰트 에셋이 `assets/node_modules/...` 경로로 생성되는데, Vercel이 경로에 `node_modules`가 포함된 폴더를 항상 자동 제외함. `dist`가 매번 새로 생성되다 보니 그때 적용한 수동 수정(`mv`+문자열 치환)이 이번엔 빠져 있었음
- 해결: `roomi-app/scripts/fix-web-deploy.js` 신규 작성 — `dist/assets/node_modules`를 `dist/assets/vendor`로 이름 변경하고 JS 번들 안의 경로 문자열도 자동 치환. `package.json`에 `npm run export:web` 스크립트 추가(`expo export -p web` + 이 스크립트를 한 번에 실행) — 앞으로는 매번 손으로 안 고쳐도 됨

## 260625 (이어서) — 날짜 범위 캘린더 공유 컴포넌트화

### 리팩토링
- 발견: 메인 검색바(`SearchBar.tsx`)의 날짜 선택 캘린더가 예약 화면(`booking/[roomId].tsx`)과 똑같은 "체크인~체크아웃 사이 미표시" 버그를 별도로 가지고 있었음 — 두 화면이 캘린더 로직(`onDayPress`, `markedDates` 계산)을 각자 중복 구현하고 있었던 것이 원인
- `components/DateRangeCalendar.tsx` 신규 — 날짜 범위 표시 로직(`getDateBetween` + `markedDates` 계산)을 한 곳으로 통합한 공유 컴포넌트
- `booking/[roomId].tsx`, `SearchBar.tsx` 양쪽 모두 자체 구현을 제거하고 `DateRangeCalendar`를 가져다 쓰도록 변경 — 같은 버그가 한쪽만 고쳐지는 문제 재발 방지
- `SearchBar.tsx`의 캘린더 모달에 "바깥 영역 누르면 닫기" 추가 — 같은 파일의 지역 선택 모달에는 이미 있던 패턴(`transparent` + 배경 `Pressable`)이 캘린더 모달에는 빠져있던 것을 발견·통일

## 260625 (이어서) — 검색바 인원 선택 UI 추가

### 기능 변경
- 인원 입력을 자유 텍스트(`TextInput`)에서 선택형(1~6명, `GUESTS` 배열)으로 변경 — 지역 선택과 동일한 하단 시트 패턴(`transparent` Modal + 배경 `Pressable`) 재사용
- `npx tsc --noEmit` 오류 0개, 인원 선택 → 검색 동작 확인

## 260625 (이어서) — 인원 선택 컴포넌트 공유화

### 리팩토링
- 검색바(고정 1~6명)와 예약 화면(객실별 실제 `maxGuests`까지)의 인원 선택 로직이 서로 다른 방식(선택형 vs 자유 입력)으로 중복 구현되어 있던 것을 통합
- `components/GuestSelector.tsx` 신규 — `max` prop으로 선택 가능한 인원 범위를 조절하는 공유 컴포넌트, 모달 열림 상태까지 컴포넌트 내부에서 자체 관리
- `SearchBar.tsx`(`max={6}`), `booking/[roomId].tsx`(`max={객실 maxGuests}`, React Hook Form `Controller`와 연동해 Zod 검증 유지) 양쪽 모두 적용
- 예약 화면 인원도 자유 입력 대신 객실 실제 수용 인원까지만 선택 가능한 방식으로 변경됨(부가 효과)

## 260625 (이어서) — 마이페이지 내 내 예약·위시리스트 바로가기 추가

### 기능 추가
- `app/(tabs)/mypage.tsx`에 "내 예약"·"위시리스트" 바로가기 버튼 2개 추가(`router.push`로 기존 탭 화면 이동), 로그아웃 버튼 위에 배치
- 메인 컬러(`sky-500`)로 로그아웃 버튼(빨강)과 구분

## 260625 (이어서) — 예약 화면에 박 수 기준 총액 표시 추가

### 기능 추가
- 발견: 백엔드(`roomi-api/app/api/bookings/route.ts`)는 이미 박 수에 비례한 총액(`room.price * nights`)을 정확히 계산해서 저장하고 있었지만, 예약 화면에서는 날짜를 바꿔도 항상 1박 가격만 고정 표시되어 사용자가 예약 전 총액을 확인할 방법이 없었음
- `app/booking/[roomId].tsx`에 체크인~체크아웃 기준 박 수·총액 계산 추가, 날짜 선택 완료 시 "n박 × 가격 = 총액" 형식으로 표시
- `npx tsc --noEmit` 오류 0개, 2박/3박 선택 시 총액 정상 표시 확인

## 260626 — 예약 화면 총액 표시 강조 개선

### 스타일 변경
- `app/booking/[roomId].tsx` — 총액 표시를 한 줄 텍스트에서 `bg-sky-50 rounded-xl` 배경 박스로 분리, 계산식(`text-sm text-gray-500`)과 합계(`text-xl font-bold text-sky-600`)를 두 줄로 나눠 총액 강조
- 커밋 `c94761b`, `feat/app` push 완료
...
### 완료
- [x] `feat/api` dev-log 커밋(`9a6e13f`) + push
- [x] `feat/app` dev-log 커밋(`f379bfa`) + push
- [x] PR 3건 생성·병합: `feat/api`→`develop`, `feat/app`→`develop`, `develop`→`main`

### 다음
1. 평점(리뷰) 작성 기능 신규
2. 프로덕션 APK 빌드 (`eas build --platform android --profile preview`)
3. 로그인 성능 개선, 카카오·네이버 OAuth (선택)
