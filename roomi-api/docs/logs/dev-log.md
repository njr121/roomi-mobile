...
## 2026-06-26 | 시드 데이터 지역 다양화 + 객실 유형 추가

### 완료

- [x] `prisma/seed.ts` — `main()` 앞에 deleteMany 4단계(위시리스트→예약→객실→숙소) 추가(중복 실행 안전 처리)
- [x] `locations` 배열 추가, `i % locations.length`로 6개 지역 균등 배분(지역당 8개 숙소, 총 48개 유지)
- [x] 객실 타입 2종 추가 — 패밀리(`family`, maxGuests: 4), 스위트(`suite`, maxGuests: 6)
- [x] `npx prisma db seed` 정상 실행 확인

### 기술 결정

- `Room.type`이 `String` 타입이라 `family`/`suite` 값 추가에 마이그레이션 불필요
- deleteMany 순서는 외래 키 제약 방향 기준 — Wishlist·Booking(자식)을 먼저 지워야 Room·Accommodation(부모) 삭제 가능

### 다음

1. 프론트에서 서울 외 지역·인원 4~6명 검색 결과 정상 반환 확인

---

## 2026-06-26 | Booking.status String → enum 변경

### 완료

- [x] `prisma/schema.prisma` — `BookingStatus` enum(`CONFIRMED`/`CANCELLED`) 추가, `Booking.status` 필드 타입 `String` → `BookingStatus @default(CONFIRMED)`으로 변경
- [x] `npx prisma migrate dev --name add-booking-status-enum` 실행, DB 적용 완료
- [x] `app/api/bookings/[id]/cancel/route.ts` — `"CANCELLED"` 문자열 2곳 → `BookingStatus.CANCELLED` enum 값으로 교체, `import { BookingStatus } from "@prisma/client"` 추가
- [x] `app/api/accommodations/route.ts` — `"cancelled"` 문자열 1곳 → `BookingStatus.CANCELLED` enum 값으로 교체, import 추가
- [x] `npx tsc --noEmit` 오류 0개, 커밋 `8a742b0` push 완료

### 기술 결정

- enum 도입으로 `status`에 허용되지 않은 값이 들어오면 컴파일 단계에서 차단됨
- 기존 DB에 `"confirmed"` 소문자로 저장된 데이터가 있었으나, 마이그레이션이 컬럼 타입을 enum으로 변환하면서 정리됨

---

## 2026-06-25 | 날짜 가용성 필터 완전 검증

### 완료
- [x] `scripts/verify-availability-filter.ts` 신규 — 숙소 하나의 객실 전체를 임시 예약으로 채운 뒤, 검색 API(`/api/accommodations`)가 그 숙소를 실제로 제외하는지 확인하고 임시 데이터를 자동 정리하는 검증 스크립트
- [x] 실행 검증: "hotel 3호점" 대상으로 객실 2개 전부 점유 → 검색 결과에서 정상 제외 확인, 임시 예약 정리 완료
- [x] 기존(2026-06-22)에는 시드 데이터 구조상 시각적으로 확인 못 했던 부분을 스크립트 기반으로 완전히 검증
